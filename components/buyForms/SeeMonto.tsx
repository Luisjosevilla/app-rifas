...
const schema = yup.object().shape({
  ...
  number:yup.number().typeError('Debe ser un número').min(4,"El número de tickets tiene que ser igual o mayor a 4").required("Número de tickets es requerido"),
...
const SeeMonto = ({searchParams,methods}:{searchParams:any,methods:any}) => {
  ...
  const onSubmit = async (data: IFormInput) => {
    ...
    form.append('cedula', data.cedula!.toString()); 
    form.append('phone', data.phone!.toString());
    form.append('transfer', data.transfer!.toString());
    form.append('file', file!);
    form.append('terms', data.terms!.toString());
 

    const nextstep=await fetch("/api/buy/step1",{
      body:form,
      method:"POST"})
      const res= await nextstep.json()
      if(nextstep.status == 500){ 
  ...
  const seeMonto=async ()=>{
    ...
    toast.success("Se ha generado el monto total a pagar!")
    router.push(`/sign-up?monto=${encodeURIComponent(monto?.monto)}&number=${encodeURIComponent(getValues("number"))}&method=${encodeURIComponent(getValues("method"))}`)
  }
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col w-full gap-4 items-center justify-center">
          ...
          <span className='text-xs text-red-600 '>{errors?.name?.message}</span>
              ...
              <div className="flex flex-row gap-2 items-center justify-center">
                  <input type="text"  {...register("name")} className={`w-[300px] border-2 border-primary/40 bg-transparent rounded-lg text-md p-2 `} />
              </div>
          ...
          <span className='text-xs text-red-600 '>{errors?.phone?.message}</span>
              ...
              <div className="flex flex-row gap-2 items-center justify-center">
                  <input defaultValue={""} type="number" {...register("phone")} className={`w-[300px] border-2 border-primary/40 bg-transparent rounded-lg text-md p-2 `}/>
              </div>
            ...
            <span className='text-xs text-red-600 '>{errors?.cedula?.message}</span>
              ...
              <div className="flex flex-row gap-2 items-center justify-center">
                  <input defaultValue={""}  type="number" {...register("cedula")} className={`w-[300px] border-2 border-primary/40 bg-transparent rounded-lg text-md p-2 `}/>
              </div>
